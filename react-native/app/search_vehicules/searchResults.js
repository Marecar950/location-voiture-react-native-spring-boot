...
function SearchResults() {
    ...
    const filterOptions = () => {
        ...
        if (manuelle) {
            filtered = filtered.filter(result => result.car.transmission === 'manuelle');
        }
        if (automatique) {
            filtered = filtered.filter(result => result.car.transmission === 'automatique');
        }
    ...
    const handleSubmit =  async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/search`, formDataToSend, {
                headers: {
            ...
            <View>
                {filteredResults.map((result, index) => (
                    <View key={index} style={styles.card}>
                        <Image source={{ uri: `${API_URL}/uploads/${result.car.image}`}} style={styles.carImage} />

                        <View style={styles.cardBody}>
                            <Text style={styles.cardTitle}>{result.car.marque}</Text>
                            <View style={styles.detailsContainer}>
                                <FontAwesome6 name="gas-pump" size={25} />
                                <Text style={styles.details}>{result.car.carburant}</Text>
                                <Icon name="speedometer" size={25} />
                                <Text style={styles.details}>{result.car.kilometrage} km</Text>
                            </View>
                            <View style={styles.detailsContainer}>        
                                <FontAwesome5 name="user-friends" size={25} />
                                <Text style={styles.details}>{result.car.passagers}</Text>
                                <MaterialCommunityIcons name="car-shift-pattern" size={25} />
                                <Text style={styles.details}>{result.car.transmission}</Text>
                            </View>    
                            <View style={styles.priceContainer}>
                                <Text style={styles.price}>Total pour {result.nombreDeJours} jours : { result.prixTotal} €</Text>
                            </View>                
                        </View>
