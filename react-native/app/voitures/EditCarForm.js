...
function EditCarForm() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const location = voiture.locations[0];
                const { lieuDepart, dateDebut, dateFin } = location;
                ...
                if (ref.current) {
                    ref.current.setAddressText(lieuDepart);
                }
            } catch (error) {
                console.error(error);
            }
    ...
    const handleSubmit = async () => {
        ...
        for (let key in voitureDetails) {
            ...
            } else if(key === 'image' && voitureDetails.imagePreview) {
                formData.append(key, voitureDetails.image);
            } else if (key !== 'image') {
        ...
        try {
            const response = await axios.post(`${API_URL}/car/edit/${id}`, formData, {
                headers: {
              ...
              {!submitted ? (
                ...
                <View style={styles.inputGroup}>
                    ...
                    <View style={styles.pickerStyle}>
                        <Picker mode='dropdown' selectedValue={voitureDetails.carburant} onValueChange={(itemValue) => handleChange('carburant', itemValue)}>
                ...
                <View style={styles.inputGroup}>
                    ...
                    <TextInput style={styles.input} placeholder="Entrez le nombre de passagers" keyboardType='numeric' value={String(voitureDetails.passagers)} onChangeText={(text) => handleChange('passagers', text)} />
                </View>
                ...
                <View style={styles.inputGroup}>
                    ...
                    {voitureDetails.image && (
                        <Image source={{ uri: `http://192.168.1.136:8080/uploads/${voitureDetails.image}` }} style={styles.image} />
                    )}
                </View>
